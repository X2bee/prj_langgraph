...
def character_make_node(state: PersonaState):
    ...
    prompt_with_tools = ChatPromptTemplate.from_messages([
        ...
        If no information is available, randomly generate and return the character's attributes.\n
        Based on the values entered by the user, envision and present the character, including the character's age, gender, job, location, interests, hobbies and etc.\n
        If your character has a favourite brand of goods, etc., create a history for it as well. \n
        And if you have difficulty creating an appropriate character, use an online search to solve the problem.\n
        The returned value must be in Korean.\n
        """),
        ("human", "Input: {human_input}\n Retrieve: {context}"),
    ])
...
class Persona_Output(TypedDict):
    ...
    character_brand_prefer: Annotated[Optional[str], None, "Product Brand that the persona might prefer"]
...
def persona_setup_node(state: PersonaState):
    messages = [
        ...
        From the information given, extract the character's preferred brand name, if any.
    ...
    print("================================= Persona Setup =================================")
    print(f"성별: {response.get('character_sex', '')}")
    print(f"나이: {response.get('character_age', '')}")
    print(f"거주지: {response.get('character_location', '')}")
    print(f"흥미: {response.get('character_interest', '')}")
    print(f"취미: {response.get('character_hobby', '')}")
    print(f"직업: {response.get('character_job', '')}")
    print(f"추가정보: {response.get('character_information', '')}")
    print(f"선호 브랜드: {response.get('character_brand_prefer', '')}")
...
examples = [
    {"input": 
        """
            ...
            User Preferred Product Brands: 입생로랑, 구찌
        """, 
    "output": 
        ['피부진정용 필링패드', '수분에센스', '스틱형 파운데이션', '강아지 간식', '강아지용 배변패드', '강아지 장난감', '입생로랑 립스틱', '구찌 가방']
    },
]
...
def search_setence_node(state: SearchQueryState):
    prompt = ChatPromptTemplate.from_messages([
        ...
        ("human", """
         ...
         User Preferred Product Brands: {brand_prefer},
         """),
    ])
    ...
    response = chain.invoke(
        {
            "sex": state['character_persona_dict'].get('character_sex', ''),
            "age": state['character_persona_dict'].get('character_age', ''),
            "location": state['character_persona_dict'].get('character_location', ''),
            "interest": state['character_persona_dict'].get('character_interest', ''),
            "hobby": state['character_persona_dict'].get('character_hobby', ''),
            "job": state['character_persona_dict'].get('character_job', ''),
            "information": state['character_persona_dict'].get('character_information', ''),
            "brand_prefer": state['character_persona_dict'].get('character_brand_prefer', '')
        }
    )
...
# ----------------------------------------------------------------------------
# 노드 5. 쿼리를 선택하는 노드임.
class QuerySelect_Output(TypedDict):
    """
    Sturctured_output을 생성하기위한 클래스
    """
    query: Annotated[str, ..., "One most relevant query you would want to search for in an online shop today"]
    
query_select_model = ChatOpenAI(model="gpt-4o-mini", temperature=0.7)
query_select_model = query_select_model.with_structured_output(QuerySelect_Output)

def query_select_node(state: SearchQueryState):
    prompt = ChatPromptTemplate.from_messages([
        ("system","""
            You've become a person with a given persona.
            From the list of queries given, select and return the most appropriate query that you would want to search for in an online store today.
        """),
        ("human", """
            User Sex: {sex},
            User Age: {age},
    ...
    chain = prompt | query_select_model
    ...
    return {"selected_query": response["query"]}

# ----------------------------------------------------------------------------
# ---------------------------------- X2BEE ----------------------------------
# ----------------------------------------------------------------------------
# 노드 6. 주어진 쿼리를 바탕으로 X2BEE에 접속하고 로그인.

def x2bee_login_node(state: SearchQueryState):
    print("=============================== Enter X2BEE ===============================")
...
# ---------------------------------- X2BEE ----------------------------------
# ----------------------------------------------------------------------------
# 노드 7. 주어진 쿼리를 바탕으로 X2BEE에 접속하고 로그인. 이후 상품 선택.

class ItemSelect_Output(TypedDict):
    """
    Sturctured_output을 생성하기위한 클래스
    """
    item: Annotated[str, ..., "One most relevant item you would want to buy for in an online shop today"]
    index: Annotated[int, ..., "The index number of the instrument you selected"]
    reason: Annotated[str, ..., "Explain why your persona would want to buy it"]
    
item_select_model = ChatOpenAI(model="gpt-4o", temperature=0.5)
item_select_model = item_select_model.with_structured_output(ItemSelect_Output)

def item_select_node(state: SearchQueryState):
    driver = state['chrome_driver']
    print("=============================== X2BEE Search ===============================")    
    search_item(driver=driver, text=state["selected_query"])
    item_list = search_product_list(driver=driver)
    item_information_list = []
    for idx, item in enumerate(item_list):
        item_information = f"인덱스: {idx}, 상품명: {item[0]}, 가격: {item[1]}"
        print(item_information)
        item_information_list.append(item_information)
        
    item_information_str = "\n".join(item_information_list)
    prompt = ChatPromptTemplate.from_messages([
        ("system","""
            You are a person with a given persona.
            Imagine you're browsing an online mall based on a given search query.
            Find and present one product that you think is the best fit for the given search query and your persona.
            Also, explain why you chose that product.
            All answers must be written in Korean.
        """),
        ("human", """
            Search Query entered by the user: {query},
            User Sex: {sex},
            User Age: {age},
            User Location: {location},
            User Interest: {interest},
            ...
            User Information: {information},
            User Preferred Product Brands: {brand_prefer},
            Product List: {item_information_str}
            """),
        ])
    chain = prompt | item_select_model
    ...
    response = chain.invoke(
        {
            ...
            "information": state['character_persona_dict'].get('character_information', ''),
            "brand_prefer": state['character_persona_dict'].get('character_brand_prefer', ''),
            "item_information_str": item_information_str,
        }
    )
    print("=============================== Item Select ===============================")
    print(f"선택한 상품: {response['item']}")
    print(response['reason'])
    print(response["index"])
    print(item_list[response["index"]])
    
    return {"selected_item": response['item'], "item_index": response["index"], "select_reason": response['reason'], "item_information_list": item_information_list, "item_information_origin": item_list, "chrome_driver": driver}

# ----------------------------------------------------------------------------
# ---------------------------------- X2BEE ----------------------------------
# ----------------------------------------------------------------------------
# 노드 8. 상품 구매하기 클릭하고 상품 구매 진행하는 노드임.
def item_buy_node(state: SearchQueryState):
    driver = state['chrome_driver']
    driver.get(state['item_information_origin'][state['item_index']][2])
    click_close(driver=driver)
